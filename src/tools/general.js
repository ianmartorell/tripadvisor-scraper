...
async function getReviewsForAttraction(locationId) {
    ...
    revs.forEach(review => reviews.push(processReview(review)));
    ...
    if (data.paging && data.paging.next) {
        ...
        for (let i = 0; i <= numberOfRuns; i++) {
            ...
            reviewsToPush.forEach(review => reviews.push(processReview(review)));
            if (shouldSlice) break;
        }
    }
    return reviews;
...
async function getAttractionDetail(attraction) {
    ...
    const reviews = await getReviewsForAttraction(locationId);
